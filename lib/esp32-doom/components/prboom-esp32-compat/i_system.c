...
// WAD embedded in firmware via board_build.embed_files = doom1-cut.wad.
// The linker exposes these symbols; the data lives in DROM (already mapped).
extern const uint8_t _binary_doom1_cut_wad_start[];
extern const uint8_t _binary_doom1_cut_wad_end[];
...
int I_Open(const char *wad, int flags) {
	int x=3;
	while (fds[x].part!=NULL || fds[x].is_embedded) x++;
	if (strcmp(wad, "DOOM1.WAD")==0) {
		fds[x].part = NULL;
		fds[x].is_embedded = 1;
		fds[x].offset = 0;
		fds[x].size = (int)(_binary_doom1_cut_wad_end - _binary_doom1_cut_wad_start);
	} else {
...
void *I_Mmap(void *addr, size_t length, int prot, int flags, int ifd, off_t offset) {
	// Embedded blob is already in DROM — just return the pointer.
	if (fds[ifd].is_embedded) {
		return (void*)(_binary_doom1_cut_wad_start + offset);
	}
...
int I_Munmap(void *addr, size_t length) {
	// Embedded blob has no handle to release.
	if ((const uint8_t*)addr >= _binary_doom1_cut_wad_start &&
	    (const uint8_t*)addr <  _binary_doom1_cut_wad_end) {
		return 0;
	}
...
// access() stub removed — the VFS implementation in libvfs.a is used instead.
// prboom only calls access() for optional DEH patches and screenshots, not the WAD.
