...
void I_SetPalette (int pal)
{
	...
	palette+=pal*(3*256);
	for (i=0; i<255 ; i++) {
		/* Standard RGB565: palette[0]=R, palette[1]=G, palette[2]=B.
		   No byte-swap here — TFT_eSPI (setSwapBytes false) sends high byte first,
		   which is what the ST7789 expects. */
